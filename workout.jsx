...
const WorkoutLogger = () => {
    ...
    const fetchWorkoutsByCategory = async (categoryId) => {
        ...
        try {
            ...
            const uniqueWorkouts = response.data.reduce((acc, workout) => {
                const existingWorkout = acc.find(w =>
                    w.exercises.map(e => e.name).sort().join(', ') ===
                    workout.exercises.map(e => e.name).sort().join(', ')
                );
                if (!existingWorkout) {
                    acc.push(workout);
                }
    ...
    const handleLogWorkout = async () => {
        ...
        try {
            ...
            const endpoint = selectedWorkout ? 'updateWorkout' : 'logWorkout';
            await axios.post(`${baseURL}api/workout/${endpoint}`, payload);
    ...
    const handleDeleteWorkout = async (workoutId) => {
        ...
        try {
            await axios.delete(`${baseURL}api/workout/deleteWorkout`, {
                data: { workoutId, email }
            });
            ...
            setError('Error deleting workout: ' + (error.response?.data?.message || error.message));
        }
    };
